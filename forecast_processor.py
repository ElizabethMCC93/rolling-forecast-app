...
try:
    ...
    st.warning("⚠️ statsmodels not available. Exponential Smoothing will use simplified version.")
...
try:
    ...
    st.warning("⚠️ statsmodels.SARIMAX not available. ARIMA will use simplified version.")
...
class ForecastProcessor:
    ...
    def ejecutar_forecast(self, modelos_ejecutar: Dict[str, bool]) -> Dict[str, Any]:
        ...
        if modelos_ejecutar.get('media_movil', False):
            with st.spinner("📈 Executing Moving Average (Seasonal)..."):
    ...
    def _get_seasonal_window(self, forecast_date: datetime, historical_series: pd.Series) -> List[float]:
        ...
        Args:
            ...
            historical_series: Historical time series
            
        Returns:
            List with 3 values (or less if not available)
        """
        ...
        # Try to get values from historical series
    ...
    # ============================================
    # MOVING AVERAGE MODEL (SEASONAL)
    # ============================================
    
    def _ejecutar_media_movil(self, datos: Dict[str, Any]) -> Dict[str, Any]:
        """
        Execute Seasonal Moving Average model
        For each forecast month, uses same month last year + 2 following months
        ...
        try:
            ...
            p2p_col = col_names.get('main_p2p')  # Check if P2P column exists in Main
            ...
            for idx in range(len(df_main)):
                ...
                # Process each forecast month independently
                for forecast_date in datos['forecast_dates']:
                    ...
                    # Handle "Depende do mês de Lançamento"
                    ...
                    # Handle "DE PARA SEGUINTE" (P2P)
                    if calc_base in [self.CALC_BASE_P2P, self.CALC_BASE_LAUNCH_DEPENDENT]:
                        
                        # Get historical series (from P2P model if specified)
                        if pd.notna(p2p_model) and p2p_model != '':
                            # Use P2P model from LogicsxMonth
                            historical_series = self._get_p2p_series_indexed(customer, p2p_model, datos)
                        else:
                        ...
                        if len(historical_series) > 0:
                            # Get seasonal window (same month last year + 2 following)
                            seasonal_values = self._get_seasonal_window(forecast_date, historical_series)
                            
                            if len(seasonal_values) > 0:
                                # Calculate average
                                seasonal_avg = np.mean(seasonal_values)
                                
                                # Apply growth factor for the forecast year
                                ...
                                # No seasonal data available, use simple average of all historical
                                simple_avg = historical_series.mean()
                                year = forecast_date.year
                                growth_factor = self._get_growth_factor(customer, year, datos)
                                df_result.loc[idx, forecast_date] = max(0, simple_avg * growth_factor)
                                cells_updated += 1
                        else:
                            df_result.loc[idx, forecast_date] = 0
                            cells_updated += 1
    ...
    def _ejecutar_suavizacao(self, datos: Dict[str, Any]) -> Dict[str, Any]:
        """
        Execute Exponential Smoothing with Holt-Winters (seasonal)
        Projects 18 months considering seasonality
        ...
        try:
            ...
            for idx in range(len(df_main)):
                ...
                # Get one logic check (use first forecast month)
                calc_base, p2p_model, launch_month = self._get_logic_for_product(
                    customer, product_class, datos['forecast_dates'][0], datos
                )
                ...
                if len(historical_series.dropna()) >= 12:  # Need at least 1 year
                    
                    try:
                        if STATSMODELS_AVAILABLE:
                            # Use Holt-Winters with seasonal component
                            ...
                            if len(clean_series) >= 12:
                                ...
                                # Assign to corresponding months
                                for i, forecast_date in enumerate(datos['forecast_dates']):
                                    if i < len(forecast_values):
                                        df_result.loc[idx, forecast_date] = max(0, forecast_values[i])
                                        cells_updated += 1
                            ...
                            for forecast_date in datos['forecast_dates']:
                                df_result.loc[idx, forecast_date] = max(0, last_value)
                                cells_updated += 1
                            
                            processed_products += 1
                    
                    except Exception as e:
                        # If Holt-Winters fails, use simple average
                        avg_value = historical_series.mean()
                        for forecast_date in datos['forecast_dates']:
                            df_result.loc[idx, forecast_date] = max(0, avg_value)
                            cells_updated += 1
                        processed_products += 1
    ...
    def _ejecutar_arima(self, datos: Dict[str, Any]) -> Dict[str, Any]:
        """
        Execute SARIMA (Seasonal ARIMA) model
        Uses 2 years of history with seasonal component
        ...
        try:
            ...
            for idx in range(len(df_main)):
                ...
                if len(historical_series.dropna()) >= 18:  # Need at least 18 months
                    ...
                    try:
                        if SARIMAX_AVAILABLE:
                            ...
                            # Simplified trend-based forecast
                            clean_series = historical_series.dropna()
                            if len(clean_series) >= 6:
                                ...
                                for i, forecast_date in enumerate(datos['forecast_dates']):
                                    ...
                                    df_result.loc[idx, forecast_date] = max(0, forecast_value)
                                    cells_updated += 1
                                
                                processed_products += 1
                    
                    except Exception as e:
                        # If SARIMA fails, use average
                        avg_value = historical_series.mean()
                        for forecast_date in datos['forecast_dates']:
                            df_result.loc[idx, forecast_date] = max(0, avg_value)
                            cells_updated += 1
            ...
            return {
                ...
                'parametros': {
                    ...
                    'seasonal_order': '(1,1,1,12)' if SARIMAX_AVAILABLE else 'N/A'
                },
                'metadata': {
                    'n_products_processed': processed_products,
                    'forecast_months': len(datos['forecast_dates']),
    ...
    def _get_historical_series_indexed(self, row_idx: int, datos: Dict) -> pd.Series:
        """
        Get historical time series for a product row with datetime index
    ...
    def _get_p2p_series_indexed(self, customer: str, p2p_model: str, datos: Dict) -> pd.Series:
        """
        Get historical series from P2P model with datetime index
        ...
        Returns:
            Time series from P2P model with datetime index
        """
        df_main = datos['df_main']
        col_names = datos['col_names']
